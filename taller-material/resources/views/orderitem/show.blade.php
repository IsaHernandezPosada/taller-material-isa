...
<div class="container mt-4">
    ...
    <div class="card">
        <div class="card-body">
            ...
            <div class="mb-3">
                ...
                <p>{{ $viewData['orderitem']->getId() }}</p>
            </div>

            <div class="mb-3">
                <label class="fw-bold">Unit Price:</label>
                <p>{{ $viewData['orderitem']->getUnitPrice() }}</p>
            </div>

            <div class="mb-3">
                <label class="fw-bold">Quantity:</label>
                <p>{{ $viewData['orderitem']->getQuantity() }}</p>
            </div>

            <div class="mb-3">
                <label class="fw-bold">Subtotal:</label>
                <p>{{ $viewData['orderitem']->getSubtotal() }}</p>
            </div>

            <div class="mb-3">
                <label class="fw-bold">Order ID:</label>
                <p>{{ $viewData['orderitem']->getOrderId() }}</p>
            </div>

            <div class="mb-3">
                <label class="fw-bold">Piece ID:</label>
                <p>{{ $viewData['orderitem']->getPieceId() }}</p>
            </div>
    ...
    <div class="mt-3">
        ...
        <form action="{{ route('orderitems.destroy', $viewData['orderitem']->getId()) }}" method="POST" style="display:inline;">
            @csrf
            @method('DELETE')
